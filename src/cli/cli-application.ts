...
import { RelationshipDetector } from "../analyzers/relationship-detector";
import { GraphGenerator } from "../generators/graph-generator";
import { ContextFileGenerator } from "../generators/context-file-generator";
import { RelationshipsJsonGenerator } from "../generators/relationships-json-generator";
...
export class CliApplication {
  ...
  private async generateCode(
    ...
    try {
      ...
      if (args.verbose) {
        displayInfoMessage("Generating services...");
      }
      serviceGen.generate(document, project, args.output, structureConfig);

      // Generate Phase 9 features if --graph flag is enabled
      if (args.graph) {
        if (args.verbose) {
          displayInfoMessage("Generating relationship graph (Phase 9 feature)...");
        }
        ...
        graphGen.generate(graph, args.output);

        if (args.verbose) {
          displayInfoMessage("Generating context files (Phase 9 feature)...");
        }

        // Generate context files
        const contextGen = new ContextFileGenerator(graph, document);
        contextGen.generateAll(args.output);

        if (args.verbose) {
          displayInfoMessage("Generating RELATIONSHIPS.json (Phase 9 feature)...");
        }

        // Generate RELATIONSHIPS.json
        const relJsonGen = new RelationshipsJsonGenerator();
        relJsonGen.generate(graph, args.output);

        displaySuccessMessage(
          "Generated relationship graph, context files, and RELATIONSHIPS.json"
        );
      }
