 ...
 * `/pets/{petId}/toys` → "Pets", `/store/inventory` → "Store"
 */
 ...
 * Operations without tags are grouped by their first path segment (resource).
 *
...
export function groupOperationsByTag(
  ...
  for (const operation of operations) {
    const tag = getPrimaryTag(operation) || resourceFromPath(operation.path);
