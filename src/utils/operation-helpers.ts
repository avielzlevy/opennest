...
/**
 * Extract a resource/controller name from a URL path.
 * `/pets/{petId}/toys` → "pets", `/store/inventory` → "store", `/` → "root"
 */
function resourceFromPath(pathUrl: string): string {
  const segments = pathUrl.split('/').filter(Boolean);
  const first = segments.find((s) => !s.startsWith('{'));
  if (!first) return 'Root';
  // PascalCase-ish: capitalize first letter
...
export function groupOperationsByTag(
  ...
  for (const [pathUrl, pathItem] of Object.entries(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      const tag = operation.tags?.[0] ?? resourceFromPath(pathUrl);
...
/**
 * Collects all allowed HTTP methods for each path in the OpenAPI spec.
 * Always includes HEAD alongside GET, and always includes OPTIONS for every path.
 *
 * @param paths - OpenAPI PathsObject
 * @returns Record mapping each path to its uppercase HTTP methods
 *
 * @example
 * collectAllowedMethodsByPath({
 *   '/users': { get: { responses: {} }, post: { responses: {} } }
 * })
 * // → { '/users': ['GET', 'HEAD', 'POST', 'OPTIONS'] }
 */
export function collectAllowedMethodsByPath(
  paths: OpenAPIV3.PathsObject | undefined
