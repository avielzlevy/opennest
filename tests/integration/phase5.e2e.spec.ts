...
describe("Phase 5: Validation & Error Handling - E2E Tests", () => {
  // Scenarios 1 & 2 (well-formed spec, strict/lenient mode) are covered by
  // tests/validation/spec-validator.spec.ts — no need to duplicate here.
  ...
  describe("Scenario 4: Complex spec with multiple validation issues", () => {
    it("should identify all validation issues in single pass", () => {
      ...
      const conflictWarning = result.warnings.some(
        (w) => w.code === "NESTJS_CONFLICT",
      );
      const invalidIdWarning = result.warnings.some(
        (w) => w.code === "INVALID_IDENTIFIER",
      );
  ...
  // Scenarios 5 & 6 (reference + operation validation) are covered by
  // tests/validation/spec-validator.spec.ts — no need to duplicate here.
  ...
  describe("Scenario 7: Enum value validation", () => {
    ...
    it("should accept valid enums", () => {
      ...
      expect(
        result.errors.filter((e) => e.message.includes("Enum")).length,
      ).toBe(0);
    });
  });
