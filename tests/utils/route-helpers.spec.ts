...
describe('Route Helpers', () => {
  ...
  describe('inferParameterType', () => {
    ...
    it('should return enum type when schema has enum and matching DTO property exists', () => {
      ...
      const result = inferParameterType(schema, {
        resourceName: 'Pet',
        paramName: 'status',
        schemas: schemas as any,
      });
      expect(result).toBe('PetStatus');
    });

    it('should return string when schema has enum but no matching DTO property', () => {
      const schema = { type: 'string', enum: ['asc', 'desc'] };
      const schemas = {
        Pet: {
          type: 'object',
          properties: {
    ...
    it('should return string when schema has enum but no context provided', () => {
      const schema = { type: 'string', enum: ['available', 'pending', 'sold'] };
      expect(inferParameterType(schema)).toBe('string');
    });

    it('should return string when schema has enum but schemas is undefined', () => {
      const schema = { type: 'string', enum: ['available', 'pending', 'sold'] };
      const result = inferParameterType(schema, {
        resourceName: 'Pet',
        paramName: 'status',
    ...
    it('should return string when schema has enum but resource not in schemas', () => {
      const schema = { type: 'string', enum: ['available', 'pending', 'sold'] };
      const schemas = {
        Order: {
          type: 'object',
