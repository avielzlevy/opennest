...
describe('Operation Helpers', () => {
  ...
  describe('collectAllowedMethodsByPath', () => {
    it('should collect methods for each path in uppercase', () => {
      const paths: OpenAPIV3.PathsObject = {
        '/users': {
          get: { tags: ['Users'], responses: {} },
          post: { tags: ['Users'], responses: {} },
      ...
      const result = collectAllowedMethodsByPath(paths);

      expect(result['/users']).toContain('GET');
      expect(result['/users']).toContain('POST');
    });

    it('should include HEAD alongside GET', () => {
      const paths: OpenAPIV3.PathsObject = {
        '/users': {
          get: { responses: {} },
        },
      ...
      expect(result['/users']).toContain('GET');
      expect(result['/users']).toContain('HEAD');
    });
    ...
    it('should always include OPTIONS', () => {
      ...
      expect(result['/users']).toContain('OPTIONS');
    });
    ...
    it('should handle multiple paths', () => {
      ...
      expect(result['/users']).toEqual(['GET', 'HEAD', 'OPTIONS']);
      expect(result['/orders']).toEqual(['POST', 'DELETE', 'OPTIONS']);
    });

    it('should return empty object for undefined paths', () => {
      expect(collectAllowedMethodsByPath(undefined)).toEqual({});
    });
    ...
    it('should not include HEAD when there is no GET', () => {
      const paths: OpenAPIV3.PathsObject = {
        '/users': {
          post: { responses: {} },
        },
      ...
      expect(result['/users']).not.toContain('HEAD');
    });
  });
});
