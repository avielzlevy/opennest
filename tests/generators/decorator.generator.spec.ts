...
describe("DecoratorGenerator", () => {
  ...
  describe("Basic Decorator Generation", () => {
    it("should generate decorator function with ApiOperation", () => {
      ...
      const sourceFile = project.getSourceFile("generated/decorators/pets.decorator.ts");
    ...
    it("should handle operations without operationId using fallback", () => {
      ...
      const sourceFile = project.getSourceFile("generated/decorators/health.decorator.ts");
      expect(sourceFile).toBeDefined();
  ...
  describe("Parameter Decorators", () => {
    it("should generate ApiParam for path parameters", () => {
      ...
      expect(imports?.getNamedImports().some((i) => i.getName() === "ApiParam")).toBe(true);
    });
    ...
    it("should map parameter schema types correctly (string, integer, number)", () => {
      ...
      const sourceFile = project.getSourceFile("generated/decorators/test.decorator.ts");
  ...
  describe("Request Body Decorators", () => {
    it("should generate ApiBody with DTO import for request body", () => {
      ...
      expect(dtoImport?.getNamedImports().some((i) => i.getName() === "NewPet")).toBe(true);

      // Should import ApiBody from @nestjs/swagger
      const swaggerImport = sourceFile?.getImportDeclaration(
        (d) => d.getModuleSpecifierValue() === "@nestjs/swagger"
      );
      expect(swaggerImport?.getNamedImports().some((i) => i.getName() === "ApiBody")).toBe(true);
    });
  ...
  describe("Response Decorators", () => {
    ...
    it("should handle void responses without DTO", () => {
      ...
      expect(bodyText).toContain("ApiResponse");
      expect(bodyText).toContain("status: 200");
      expect(bodyText).toContain("description: 'Success'");
  ...
  describe("Security Decorators", () => {
    it("should generate JwtDecorator for Bearer auth", () => {
      ...
      const sourceFile = project.getSourceFile("generated/decorators/secure.decorator.ts");
      ...
      const jwtImport = sourceFile?.getImportDeclaration(
        (d) => d.getModuleSpecifierValue() === "../common/decorators/auth/jwt.decorator"
      );
    ...
    it("should generate ApiKeyDecorator for API key auth", () => {
      ...
      const apiKeyImport = sourceFile?.getImportDeclaration(
        (d) => d.getModuleSpecifierValue() === "../common/decorators/auth/apiKey.decorator"
      );
    ...
    it("should handle operations without security", () => {
      ...
      const sourceFile = project.getSourceFile("generated/decorators/public.decorator.ts");
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle malformed operationIds fixture", () => {
      const fixtureContent = fs.readFileSync(
        path.join(__dirname, "../fixtures/synthetic/malformed-operationids.yaml"),
        "utf-8"
      );
      const doc = yaml.load(fixtureContent) as OpenAPIV3.Document;
      ...
      const userFile = project.getSourceFile("generated/decorators/user.decorator.ts");
      const specialFile = project.getSourceFile("generated/decorators/special.decorator.ts");
  ...
  describe("Import Organization", () => {
    it("should organize imports correctly", () => {
      ...
      expect(fullText).toContain("import { applyDecorators } from \"@nestjs/common\"");
    ...
    it("should only import needed Swagger decorators", () => {
      ...
      const sourceFile = project.getSourceFile("generated/decorators/simple.decorator.ts");
      const swaggerImport = sourceFile?.getImportDeclaration(
        (d) => d.getModuleSpecifierValue() === "@nestjs/swagger"
      );
  ...
  describe("Complete Snapshot Tests", () => {
    it("should generate correct decorator structure (snapshot)", () => {
      ...
      const sourceFile = project.getSourceFile("generated/decorators/users.decorator.ts");
